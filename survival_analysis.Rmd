...
We conduct forward-backward selection. In this model selection technique we first construct separate univariate models. Significant factors at $\alpha = 0.10$ within each univariate model are incorporated into a multivariate model. Insignificant factors at $\alpha = 0.10$ are then eliminated. We begin by constructing univariate stratified cox models for each predictor and identify significant factors.

```{r, echo = FALSE}
coxph(surv_obj ~ insured + race + bmi_group + strata(age_group), data = surv.df)
...
For women who entered the study between ages 50 and 53, there is no significant difference between the survival curves for those with insurance versus those without.
...
- Changed diabetes incidence vs incident diabetes language
- Changed plot titles
- Income is the factor that makes insurance insignificant, check for dependence of categorical variables
- Model selection process is not clear for survival models. Forward-backward selection yields the previously discovered model
